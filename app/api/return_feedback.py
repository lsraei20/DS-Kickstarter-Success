...
def feedback(df):
    ...
    if len(df.loc[0, 'title']) > (title_length + 5):
        title_feedback = "Hmm. You should remove some words from the title," \
                         " it is quite longer than most winning campaigns."
    elif len(df.loc[0, 'title']) < (title_length - 5):
        title_feedback = "Your title is shorter than the average successful" \
                         " campaign, you should try adding to it!"
    else:
        title_feedback = "Your title looks great!"

    # Description feedback
    if len(df.loc[0, 'description']) > (blurb_length + 20):
        description_feedback = "Uh oh. Your bio is longer than the average " \
                               "successful Kickstarter. We found it can be " \
                               "the most important section to your success!"
    elif len(df.loc[0, 'description']) < (blurb_length - 15):
        description_feedback = "Can you say a bit more? Most winning " \
                               "Kickstarter's have a longer bio, and we " \
                               "found it can be the most important " \
    ...
    if campaign_len(df)['cam_length'].mean().days > (campaign_len_days + 5):
        campaign_len_feedback = "Your campaign lasts quite longer than most" \
                                " winning campaigns. Believe it or not a " \
                                "shorter campaign can actually increase " \
                                "your funding!"
    elif campaign_len(df)['cam_length'].mean().days < (campaign_len_days - 5):
        campaign_len_feedback = "Some more time perhaps? your campaign is " \
                                "quite shorter than most successful " \
                                "campaigns. You should consider giving " \
                                "people more time to fund!"
    else:
    ...
    if df.loc[0, 'monetary_goal'] > (monetary_goal_median + 3000):
        monetary_feedback = "Your goal is higher than most successful " \
                            "campaigns. You should consider lowering it" \
        ...
        monetary_feedback = "Your goal is lower than average for " \
                            "successful campaigns. Why not bump it up " \
                            "a bit? (keep in mind this is just a " \
                            "suggestion and there are a lot of other" \
    ...
    if month_proba_dic[df.loc[0, "launch_date"].month] < \
        ...
        month_launched_feedback = "By looking at other posts like yours, " \
                                  "we detected that the chances of your " \
                                  "Kickstarter success may be higher if " \
                                  "you wait to post next month. " \
